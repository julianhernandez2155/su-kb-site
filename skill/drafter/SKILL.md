---
name: su-kb-page-drafter
description: >-
  Draft a finished, schema-valid Markdown page for Syracuse's public KB site from notes or a doc,
  with the exact file path and terminal-free GitHub steps. Use to author a new SU AI help/KB page.
version: 1.0.0
---

<!--
  SINGLE SOURCE OF TRUTH NOTE
  This instruction body is mirrored verbatim in skill/drafter/drafter-prompt.md
  (the no-install copy-paste fallback). If you change the drafting rules here,
  change them there too. The schema rules below MUST stay in lockstep with
  tools/check_frontmatter.py — that validator is the authoritative publish gate;
  this skill's self-check is convenience, not trust. See
  rpi/faculty-page-authoring/plan/eng.md §6 for the spec.
-->
